...
namespace CaptchaApp.App_Code
{
    public class CaptchaBLL
    {
        ...
        /// 圖片寬度
        /// </summary>
        private const int _imageWidth = 200;

        /// <summary>
        /// 圖片高度
        /// </summary>
        private const int _imageHeight = 60;
        ...
        /// 背景顏色
        /// </summary>
        private readonly static Color _backGroundColor = Color.FromArgb(72, 72, 72);
        ...
        {
            using (var bmpOut = new Bitmap(_imageWidth, _imageHeight, PixelFormat.Format24bppRgb))
            {
                ...
                int m = Math.Max(_imageWidth, _imageHeight);
                var hatchBrush = new HatchBrush(HatchStyle.LargeConfetti, Color.White, Color.White);
                for (int i = 0; i <= Convert.ToInt32(Math.Truncate(_imageWidth * _imageHeight / 30f)) - 1; i++)
                {
                    int x = _random.Next(_imageWidth);
                    int y = _random.Next(_imageHeight);
                    int w = _random.Next(m / 30);
                    int h = _random.Next(m / 30);
                ...
                using (var ms = new MemoryStream())
                {
                    ...
                    //File.WriteAllBytes(@"D:\Zenii Git\python\stock\tempCaptcha.png", bmpBytes);
                    bmpOut.Dispose();
                    ms.Dispose();
                }
        ...
        private static void InterferenceLines(ref Graphics g, int lines)
        {
            ...
            {
                var pan = new Pen(GetRandomColor(_interferenceColorDepth), 2);
